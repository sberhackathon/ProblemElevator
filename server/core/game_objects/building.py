...
class Building(object):
    def __init__(self):
        ...
        for i in range(settings.BUILDING['ELEVATORS_FOR_PASSENGER_COUNT']):
            elevator_x = settings.BUILDING['FIRST_ELEVATOR_POSITION'] + i * (
                settings.BUILDING['ELEVATOR_IN_GROUP_OFFSET'])
            elevator_y = settings.BUILDING['FIRST_FLOOR']
            first_player_elevators.append(
                Elevator(elevator_id, -elevator_x, elevator_y, settings.BUILDING['FIRST_FLOOR'],
                         settings.PLAYERS['FIRST_PLAYER_KEY']))
            elevator_id += 1
            second_player_elevators.append(
                Elevator(elevator_id, elevator_x, elevator_y, settings.BUILDING['FIRST_FLOOR'],
    ...
    def set_passenger_elevator(self, passenger, elevator):
        if (
                passenger.is_waiting_for_elevator() or passenger.is_returning()) and elevator.current_floor() == passenger.from_floor and elevator.is_filling() and not elevator.is_full():
    ...
    def spawn(self):
        ...
        first_passenger = Passenger(self.passenger_id, -passenger_x, passenger_y, floors_queue, weight,
                                    settings.PLAYERS['FIRST_PLAYER_KEY'])
        self.all_passengers.append(first_passenger)
        self.passenger_id += 1

        second_passenger = Passenger(self.passenger_id, passenger_x, passenger_y, floors_queue, weight,
                                     settings.PLAYERS['SECOND_PLAYER_KEY'])
        self.all_passengers.append(second_passenger)
        self.passenger_id += 1

    def on_tick(self):
        for e in self.players_elevators[settings.PLAYERS['FIRST_PLAYER_KEY']] + self.players_elevators[
            settings.PLAYERS['SECOND_PLAYER_KEY']]:
            e.on_tick()
