...
class World(object):
    ...
    def get_visio_state(self):
        ...
        return {
            ...
            "waiting_passengers": {
                red: {f: len(list(c)) for f, c in groupby(
                    sorted(filter(lambda p: p.is_walking_on_floor(), self.get_red_passengers()), key=lambda p: p.floor),
                    lambda p: p.floor)},
                blue: {f: len(list(c)) for f, c in groupby(
                    sorted(filter(lambda p: p.is_walking_on_floor(), self.get_blue_passengers()),
                           key=lambda p: p.floor), lambda p: p.floor)}
            }
        }
